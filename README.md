...
## 扩展性考虑
-  **跨链互操作**
  未来可通过Fabric Interoperability连接土地管理局链。
- **完善DID/VC**
  尽管现在已经做到了did、didDocument、VC、VP的生成，但是并没有完全按照DID/VC的规范来处理用户的数字身份信息（比如说VC的授权使用，权限控制等），很大一部分原因是因为该项目在设计之初是将用户的“钱包”（类比区块链的Wallet）和GRETS系统整体绑定在一起的，正规的DID/VC需要将钱包与RP分离。
  同时也正是因为这个原因，导致区块链每一笔交易的creator每个组织只有一个（因为这里的creator是在CA中实际注册的客户端，而非在GRETS中注册的用户），理想情况应该是将GRETS作为一个纯粹的交易平台，而用户的客户端、GRETS账号和CA账号一起独立作为另外一个服务。
